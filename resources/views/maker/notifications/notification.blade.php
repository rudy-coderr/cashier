...
<head>
  ...
  <title>Notifications — DAR Cashier Maker</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn-back { display: flex; align-items: center; justify-content: center; width: 44px; height: 44px; border-radius: 10px; background: rgba(255,255,255,.07); border: none; color: rgba(245,240,232,.55); cursor: pointer; text-decoration: none; transition: background .15s, color .15s; font-size: 1.1rem; flex-shrink: 0; }
    .btn-back:hover { background: rgba(255,255,255,.14); color: var(--cream); }
    ...
    /* ── SIDEBAR ── */
    .sidebar { width: 280px; flex-shrink: 0; background: var(--green-deep); border-right: 1px solid rgba(255,255,255,.07); position: sticky; top: 72px; height: calc(100vh - 72px); display: flex; flex-direction: column; overflow: hidden; }
    ...
    .sidebar-section-label { padding: 0 22px; font-size: .6rem; font-weight: 700; letter-spacing: 2px; text-transform: uppercase; color: rgba(245,240,232,.28); margin-bottom: 6px; margin-top: 12px; }
    .sidebar-nav-item { display: flex; align-items: center; gap: 11px; padding: 10px 22px; cursor: pointer; transition: background .15s; border-left: 3px solid transparent; text-decoration: none; }
    .sidebar-nav-item:hover { background: rgba(255,255,255,.04); }
    .sidebar-nav-item.active { background: rgba(45,122,79,.18); border-left-color: var(--gold); }
    ...
    .sidebar-nav-item:not(.active) .nav-icon { background: rgba(255,255,255,.07); color: rgba(245,240,232,.55); }
    .sidebar-nav-item.active .nav-icon { background: var(--gold); color: var(--green-deep); }
    .nav-label { font-size: .81rem; font-weight: 600; color: rgba(245,240,232,.7); }
    .sidebar-nav-item.active .nav-label { color: var(--cream); }
    ...
    .page-body { width: 100%; max-width: 1100px; padding: 36px 36px 60px; }

    .page-title-row { display: flex; align-items: flex-end; justify-content: space-between; margin-bottom: 22px; gap: 16px; flex-wrap: wrap; }
    ...
    /* ── SUMMARY BAR ── */
    .summary-bar { display: flex; align-items: center; gap: 16px; padding: 11px 18px; background: var(--surface); border: 1.5px solid var(--border); border-radius: 10px; margin-bottom: 20px; flex-wrap: wrap; }
    ...
    /* ── TOOLBAR ── */
    .notif-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 20px; flex-wrap: wrap; }
    .notif-filters { display: flex; gap: 8px; flex-wrap: wrap; }
    .filter-pill { padding: 6px 16px; border-radius: 20px; font-size: .75rem; font-weight: 700; cursor: pointer; border: 1.5px solid var(--border); background: var(--surface); color: var(--text-mid); transition: all .15s; letter-spacing: .3px; }
    ...
    .notif-btn-actions { display: flex; gap: 8px; }
    .btn-mark-all { display: inline-flex; align-items: center; gap: 6px; padding: 7px 16px; border: 1.5px solid var(--green-accent); border-radius: 9px; background: transparent; color: var(--green-accent); font-family: 'DM Sans', sans-serif; font-size: .76rem; font-weight: 700; cursor: pointer; transition: all .15s; }
    .btn-mark-all:hover { background: var(--green-light); }
    .btn-clear-all { display: inline-flex; align-items: center; gap: 6px; padding: 7px 16px; border: 1.5px solid #e8c5c5; border-radius: 9px; background: transparent; color: var(--red); font-family: 'DM Sans', sans-serif; font-size: .76rem; font-weight: 700; cursor: pointer; transition: all .15s; }
    .btn-clear-all:hover { background: #fdf0ef; }

    /* ── NOTIFICATION CARDS ── */
    .notif-date-header { display: flex; align-items: center; gap: 10px; font-size: .62rem; font-weight: 700; letter-spacing: 1.6px; text-transform: uppercase; color: var(--muted); margin: 24px 0 10px; }
    .notif-date-header:first-child { margin-top: 0; }
    .notif-date-header::after { content: ''; flex: 1; height: 1px; background: var(--border); }
    .notif-card { display: flex; align-items: flex-start; gap: 14px; padding: 16px 20px; background: var(--surface); border: 1.5px solid var(--border); border-radius: 12px; margin-bottom: 8px; cursor: pointer; transition: box-shadow .15s, border-color .15s, background .15s; position: relative; overflow: hidden; }
    .notif-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; background: transparent; }
    ...
    .notif-icon-wrap { width: 42px; height: 42px; border-radius: 11px; display: flex; align-items: center; justify-content: center; font-size: 1.1rem; flex-shrink: 0; }
    ...
    .notif-tag { display: inline-flex; align-items: center; gap: 4px; padding: 3px 9px; border-radius: 20px; font-size: .65rem; font-weight: 700; letter-spacing: .4px; text-transform: uppercase; }
    ...
    .tag-meta      { background: #faf8f4; color: var(--text-mid); border: 1px solid var(--border); }
    .notif-card-actions { display: flex; flex-direction: column; align-items: center; gap: 6px; flex-shrink: 0; }
    .card-action-btn { width: 30px; height: 30px; border-radius: 7px; border: 1.5px solid var(--border); background: #faf8f4; color: var(--text-mid); display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: .8rem; transition: all .15s; }
    ...
    /* ── EMPTY STATE ── */
    .notif-empty-state { text-align: center; padding: 80px 20px; display: none; }
    .notif-empty-state.show { display: block; }
    .empty-icon-wrap { width: 72px; height: 72px; border-radius: 50%; background: var(--green-light); display: flex; align-items: center; justify-content: center; margin: 0 auto 16px; font-size: 1.8rem; color: var(--green-accent); }
    ...
    .btn-confirm-red { padding: 8px 18px; border: none; border-radius: 8px; background: var(--red); color: #fff; font-family: 'DM Sans', sans-serif; font-size: .8rem; font-weight: 700; cursor: pointer; transition: background .15s; display: flex; align-items: center; gap: 6px; }
    .btn-confirm-red:hover { background: #8b1f18; }
    @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }

    @media (max-width: 768px) {
      .outer-wrapper { flex-direction: column; }
      .sidebar { width: 100%; height: auto; position: static; overflow-x: auto; border-right: none; border-bottom: 1px solid rgba(255,255,255,.07); }
      .sidebar-scroll { display: flex; overflow-x: auto; padding: 8px 0; }
      .sidebar-section-label, .sidebar-footer { display: none; }
      .sidebar-nav-item { white-space: nowrap; border-left: none; border-bottom: 2px solid transparent; }
      .sidebar-nav-item.active { border-bottom-color: var(--gold); }
    ...
    <div class="header-user-wrap" id="headerUserWrap">
      <div class="header-user" onclick="toggleHeaderDropdown()">
        ...
        <div>
          ...
          <div class="header-user-role">{{ ucfirst($authUser->position ?? $authUser->role ?? 'Maker') }}</div>
        </div>
...
<div class="outer-wrapper">
  ...
  <aside class="sidebar">
    <div class="sidebar-scroll">
      <div class="sidebar-section-label" style="margin-top:16px;">Transactions</div>
      <a class="sidebar-nav-item" href="{{ route('dashboard') }}">
        <div class="nav-icon"><i class="bi bi-house"></i></div>
        <span class="nav-label">Dashboard</span>
      </a>
     <a class="sidebar-nav-item" href="{{ route('payments.create') }}">
    <div class="nav-icon">
  ...
  <main class="main-content">
    <div class="page-body">
      ...
      <div class="page-title-row">
        ...
        <a href="{{ route('dashboard') }}" style="display:inline-flex;align-items:center;gap:6px;font-size:.78rem;font-weight:700;color:var(--green-accent);text-decoration:none;">
      ...
      <div class="notif-empty-state" id="empty-state">
        ...
        <div class="empty-sub">You're all caught up! Nothing matches the current filter.</div>
      </div>
...
<div class="modal-overlay" id="clear-modal" onclick="handleClearOverlay(event)">
  <div class="modal-box">
    ...
    <div class="modal-body-inner">
      Are you sure you want to <strong>delete all notifications</strong>? This action cannot be undone.
    </div>
...
<script>
  ...
  async function fetchAll() {
    try {
      const res = await fetch('/maker/notifications', { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
      ...
      document.getElementById('loading-state').innerHTML =
        '<i class="bi bi-exclamation-circle" style="font-size:1.4rem;display:block;margin-bottom:8px;color:var(--red);"></i> Failed to load notifications.';
    }
  }
  ...
  function timeAgo(ds) {
    ...
    try {
      ...
      return new Date(ds).toLocaleDateString('en-PH', { month:'short', day:'numeric', year:'numeric' });
    } catch(e) { return ''; }
  }
  function isToday(ds)     { if (!ds) return false; return new Date(ds).toDateString() === new Date().toDateString(); }
  function isYesterday(ds) { if (!ds) return false; const y = new Date(); y.setDate(y.getDate()-1); return new Date(ds).toDateString() === y.toDateString(); }
  ...
  function buildCard(n) {
    const c   = getCfg(n.status);
    const uCl = n.unread ? 'unread' : '';
    const dot = n.unread ? '<div class="unread-dot"></div>' : '';
    const tags = [
      `<span class="notif-tag ${c.tagCls}"><i class="bi ${c.icon}"></i> ${c.label}</span>`,
      n.fund   ? `<span class="notif-tag tag-meta"><i class="bi bi-bank2"></i> ${esc(n.fund)}</span>` : '',
      n.amount ? `<span class="notif-tag tag-meta"><i class="bi bi-cash-coin"></i> ${n.amount}</span>` : '',
    ...
    <div class="notif-card ${uCl} ${c.type}" data-id="${n.id}" onclick="readOne('${n.id}')">
      ...
      <div class="notif-content">
        <div class="notif-top">
          <div class="notif-title">Your transaction for ${esc(n.name)} — ${c.label}</div>
          <div class="notif-meta"><span class="notif-time">${timeAgo(n.created)}</span>${dot}</div>
        </div>
        <div class="notif-body">The payment record for <strong>${esc(n.name)}</strong> ${c.msg}</div>
  ...
  function render() {
    ...
    function grp(items, label) { return items.length ? `<div class="notif-date-header">${label}</div>` + items.map(buildCard).join('') : ''; }

    document.getElementById('group-today').innerHTML     = grp(today,     'Today');
    document.getElementById('group-yesterday').innerHTML = grp(yesterday, 'Yesterday');
    document.getElementById('group-older').innerHTML     = grp(older,     'Earlier');

    const empty = document.getElementById('empty-state');
    list.length === 0 ? empty.classList.add('show') : empty.classList.remove('show');
  }
  ...
  async function markOneRead(id) {
    ...
    const n = ALL_NOTIFS.find(x => x.id === id); if (n) n.unread = false;
    render();
  }

  async function markAllRead() {
    try { await fetch('{{ route('maker.notifications.mark_all') }}', { method:'POST', headers:{ 'Content-Type':'application/json','X-CSRF-TOKEN':CSRF }, credentials:'same-origin' }); } catch(e) {}
    ALL_NOTIFS.forEach(n => n.unread = false);
    render();
  }

  async function deleteOne(id) {
    try { await fetch('/maker/notifications/' + id, { method:'DELETE', headers:{ 'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':CSRF } }); } catch(e) {}
  ...
  async function clearAllNotifications() {
    closeClearModal();
    try { await fetch('{{ route('maker.notifications.clear_all') }}', { method:'DELETE', headers:{ 'Content-Type':'application/json','X-CSRF-TOKEN':CSRF }, credentials:'same-origin' }); } catch(e) {}
    ALL_NOTIFS = [];
    render();
  }
    ...
    try {
      document.querySelectorAll('form[action="{{ route('logout') }}"]').forEach(f => {
        f.addEventListener('submit', function(ev) {
          ev.preventDefault();
          Swal.fire({ title:'Log out?', text:'Are you sure you want to log out?', icon:'warning', showCancelButton:true, confirmButtonText:'Yes, log out', cancelButtonText:'Cancel' })
