...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* Back btn */
    .btn-back { display: flex; align-items: center; justify-content: center; width: 44px; height: 44px; border-radius: 10px; background: rgba(255,255,255,.07); border: none; color: rgba(245,240,232,.55); cursor: pointer; text-decoration: none; transition: background .15s, color .15s; font-size: 1.1rem; }
    ...
    .notif-card::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 3px; background: transparent; transition: background .15s; }
    ...
    @media (max-width: 1024px) { .page-body { max-width: 100%; } }
    ...
    <div class="page-body">
      ...
      <div class="summary-bar">
        ...
        <div class="summary-item">
          ...
          <span>Forwarded: <span class="summary-count" id="count-forwarded">0</span></span>
        </div>
        <div class="summary-sep"></div>
        <div class="summary-item">
          <div class="summary-dot" style="background:var(--green-accent);"></div>
      ...
      <div class="notif-empty-state" id="empty-state">
        ...
        <div class="empty-sub">You're all caught up! Nothing here for the selected filter.</div>
      </div>
...
<script>
  ...
  async function fetchAll() {
    try {
      ...
      ALL_NOTIFS = raw.map(n => ({
        id:      n.id,
        unread:  !(n.read === true),
        status:  (n.data?.status || 'forwarded').toLowerCase(),
  ...
  function isYesterday(ds) {
    if (!ds) return false;
    const y = new Date(); y.setDate(y.getDate() - 1);
    return new Date(ds).toDateString() === y.toDateString();
  }
  ...
  function buildCard(n) {
    ...
    const tags  = [
      ...
      n.amount ? `<span class="notif-tag tag-meta"><i class="bi bi-cash-coin"></i> ${esc(n.amount)}</span>` : '',
    ...
    <div class="notif-card ${uCls} ${cfg.type}" data-id="${n.id}" data-status="${n.status}" onclick="readOne('${n.id}')">
      ...
      <div class="notif-content">
        <div class="notif-top">
          <div class="notif-title">Payment from ${esc(n.name)} — ${cfg.label}</div>
          <div class="notif-meta">
            <span class="notif-time">${timeAgo(n.created)}</span>
            ${dot}
          </div>
        </div>
        <div class="notif-body">The payment record for <strong>${esc(n.name)}</strong> ${cfg.msg}</div>
